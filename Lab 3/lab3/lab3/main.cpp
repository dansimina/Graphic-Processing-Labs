...
//
GLfloat vertexCoordinates_secondTriangle[] = {
    -0.5f, 0.5f, 0.0f,
    0.5f, -0.5f, 0.0f,
    0.5f, 0.5f, 0.0f
};
...
//
GLuint verticesVBO_secondTriangle;
GLuint triangleVAO_secondTriangle;
...
void initObjects()
{
    ...
    //
    //genereaza un ID unic pentru verticesVBO 
    glGenBuffers(1, &verticesVBO_secondTriangle);
    //asociaza buffer-ul verticesVBO variabilei OpenGL GL_ARRAY_BUFFER, 
    //orice referire ulterioara la GL_ARRAY_BUFFER va configura buffer-ul asociat momentan, 
    //care este verticesVBO 
    glBindBuffer(GL_ARRAY_BUFFER, verticesVBO_secondTriangle);
    //copiaza datele in buffer-ul current asociat – specificat prin intermediul primului argument 
    //tipul buffer-ului – al doilea argument specifica dimensiunea (in Bytes) datelor 
    //al treilea argument reprezinta datele pe care vrem sa le trimitem  
    //al patrulea argument specifica modul in care vor fi tratate datele de catre placa video 
    glBufferData(GL_ARRAY_BUFFER, sizeof(vertexCoordinates_secondTriangle), vertexCoordinates_secondTriangle, GL_STATIC_DRAW);

    //genereaza un ID unic, care corespunde  obiectului triangleVAO 
    glGenVertexArrays(1, &triangleVAO_secondTriangle);
    glBindVertexArray(triangleVAO_secondTriangle);
    glBindBuffer(GL_ARRAY_BUFFER, verticesVBO_secondTriangle);
    //seteaza pointer-ul atributelor de varf  
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(0);
...
void renderScene()
{
    GLuint shader1 = shaderProgram;
    GLuint shader2 = shaderProgram_green;
...
int main(int argc, const char* argv[]) {
    ...
    shaderProgram_green = initBasicShader("shaders/shader_green.vert", "shaders/shader_green.frag");
